...
const Home = () => {
    ...
    const [isAnalysisComplete, setIsAnalysisComplete] = useState(false); // New state to track analysis completion

    const handleAnalyze = async () => {
        setIsAnalyzing(true);
        setIsAnalysisComplete(false); // Reset analysis completion state
        ...
        try {
            ...
            if (data.success) {
                setResponseData(data);
                setIsAnalysisComplete(true); // Mark analysis as complete
            }
        } catch (error) {
            console.error("Error during analysis:", error);
        } finally {
            setIsAnalyzing(false); // Stop loader regardless of success or failure
        }
    };
    ...
    if (isAnalysisComplete && responseData) {
        // Render ChartPage if analysis is complete and data is available
        return <ChartPage data={responseData} />;
    }
    ...
    return (
        <>
            ...
            {responseData && !isAnalysisComplete && (
                <div className="flex justify-center mt-5">
                    {/* Render chart preview if response data is available */}
                    <div className="w-2/3 bg-neutral-900 flex h-96 overflow-hidden rounded-md p-5">
                        <div className="w-[50%] overflow-scroll ">
                            {responseData?.videoList?.map((item, index) => (
                                <div
